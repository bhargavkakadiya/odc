...
def run_prophet(local=False):
    ...
    df['date'] = pd.to_datetime(
        df['date'], format='%d/%m/%Y')  # Convert to datetime
    # Impute missing values with previous and next values
    df = df.groupby(['cabin_name', 'pollutant_code']).apply(
        lambda group: group.interpolate(method='ffill', axis=1))
    df = df.groupby(['cabin_name', 'pollutant_code']).apply(
        lambda group: group.interpolate(method='bfill', axis=1))
    ...
    for cabin in df_.groups.keys():
        ...
        days_to_predict = (datetime.datetime(
             2023, 2, 28) - df_.get_group(cabin).date.max()).days
